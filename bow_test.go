...
func TestBow_NewSlice(t *testing.T) {
	origin, err := NewBowWithMetadata(NewMetadata([]string{"k"}, []string{"v"}),
		NewPrevSeries("time", Int64, []int64{1, 2, 3}, nil),
		NewPrevSeries("value", Float64, []float64{.1, .2, .3}, nil),
	)
	require.NoError(t, err)

	// begin
	expected, err := NewBowWithMetadata(NewMetadata([]string{"k"}, []string{"v"}),
		NewPrevSeries("time", Int64, []int64{1}, nil),
		NewPrevSeries("value", Float64, []float64{.1}, nil),
	)
	require.NoError(t, err)
	...
	// end
	expected, err = NewBowWithMetadata(NewMetadata([]string{"k"}, []string{"v"}),
		NewPrevSeries("time", Int64, []int64{2, 3}, nil),
		NewPrevSeries("value", Float64, []float64{.2, .3}, nil),
	)
	require.NoError(t, err)
	...
	expected, err = NewBowWithMetadata(NewMetadata([]string{"k"}, []string{"v"}),
		NewPrevSeries("time", Int64, []int64{}, nil),
		NewPrevSeries("value", Float64, []float64{}, nil),
	)
	require.NoError(t, err)
...
func TestBow_Select(t *testing.T) {
	...
	t.Run("with metadata", func(t *testing.T) {
		b, err := NewBowWithMetadata(NewMetadata([]string{"k"}, []string{"v"}),
			NewPrevSeries("time", Int64, []int64{1, 2, 3}, []bool{true, false, true}),
			NewPrevSeries("value", Float64, []float64{1, 2, 3}, []bool{true, false, true}),
		)
		require.NoError(t, err)
...
func TestBow_DropNils(t *testing.T) {
	...
	t.Run("with metadata", func(t *testing.T) {
		...
		expected, err := NewBowWithMetadata(NewMetadata([]string{"k"}, []string{"v"}),
			NewPrevSeries("time", Int64, []int64{1, 3}, nil),
			NewPrevSeries("value", Float64, []float64{1, 3}, nil),
		)
		require.NoError(t, err)
...
func TestBow_AddCols(t *testing.T) {
	...
	require.NoError(t, err)
	serieC := NewPrevSeries("c", Int64, []int64{1, 2, 3, 4}, nil)
	serieD := NewPrevSeries("d", String, []string{"one", "two", "three", "four"}, nil)
	serieE := NewPrevSeries("e", Boolean, []bool{true, false, true, false}, nil)
	...
	t.Run("column name already exists", func(t *testing.T) {
		_, err := bow1.AddCols(NewPrevSeries("a", Int64, []int64{1, 2, 3, 4}, nil))
		require.Error(t, err)
