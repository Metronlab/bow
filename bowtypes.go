...
var (
	...
	mapArrowNameToBowType map[string]Type
)

func init() {
	mapBowToArrowTypes = make(map[Type]arrow.DataType)
	mapArrowNameToBowType = make(map[string]Type)
	for arrowDataType, bowType := range mapArrowToBowTypes {
		mapBowToArrowTypes[bowType] = arrowDataType
		mapArrowNameToBowType[arrowDataType.Name()] = bowType
	}
}
...
func getBowTypeFromArrowName(arrowName string) Type {
	t, ok := mapArrowNameToBowType[arrowName]
	if !ok {
		return Unknown
	}
	return t
