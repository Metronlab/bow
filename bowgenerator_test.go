...
func TestGenerator(t *testing.T) {
	t.Run("default", func(t *testing.T) {
		...
		assert.Equal(t, Int64, bow.GetColType(0))
	...
	t.Run("float64 with first column sorted", func(t *testing.T) {
		...
		assert.Equal(t, Float64, bow.GetColType(0))
		assert.Equal(t, Float64, bow.GetColType(1))
	...
	t.Run("custom names and types", func(t *testing.T) {
		...
		assert.Equal(t, Int64, bow.GetColType(0))
		assert.Equal(t, Float64, bow.GetColType(1))
		assert.Equal(t, String, bow.GetColType(2))
		assert.Equal(t, Bool, bow.GetColType(3))
	})
}
