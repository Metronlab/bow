...
func TestParquet(t *testing.T) {
	...
	t.Run("ttt", func(t *testing.T) {
		...
		series[0] = NewSeries("time", []int64{0}, []bool{true})
		series[1] = NewSeries(">\"<", []float64{0.}, []bool{true})
		...
		context := Context{
			"time":                    Meta{Unit{Symbol: "microseconds"}},
			html.EscapeString(">\"<"): Meta{Unit{Symbol: "kWh"}},
		}
		...
		err = bBefore.WriteParquet(testOutputFileName+"_ttt", true)
		assert.NoError(t, err)

		bAfter, err := NewBowFromParquet(testOutputFileName+"_ttt.parquet", true)
		assert.NoError(t, err)
